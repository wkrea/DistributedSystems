...
class StopReview(db.Model):
    __tablename__ = "stopreviews"
    ...
    def __repr__(self):
        return "<StopReview id={}, stop_id={}, reviewer_id={}, rating={}>".format(self.id, self.stop_id, self.reviewer_id, self.rating)
...
class CreateReview(Resource):
    """
        API entrypoint to create stop review.
    """
    ...
    def post(self):
        try:
            ...
            # check if form is ok
            if not 'stopId' in request.form:
                # stopId not in form
                return {
                    'causeErrorMessage': 'N/A',
                    'causeErrorStatus': -1,
                    'errorMessage': "Missing 'stopId' entry in HTTP request body.",
                    'responseCode': 400
                }, 400
            ...
            stop_id    = request.form["stopId"]
            stop_score = request.form['score']

            # place in DB
            new_review = StopReview(stop_id=stop_id, reviewer_id=auth_data['userId'], score=stop_score)
            ...
            return {
                ...
                'causeErrorStatus': -1,
                'errorMessage': 'Cannot add review: review for specified user and stop already exists!', 
                'responseCode': 400
            }, 400
...
class GetScore(Resource):
    """
        API entrypoint to retrieve the average score of a stop.
    """

    def get(self, stop_id):
        try:
            reviews = StopReview.query.filter_by(stop_id=stop_id).all()

            if len(reviews) == 0:
                return {
                    'causeErrorMessage': 'N/A',
                    'causeErrorStatus': -1,
                    'errorMessage': 'Specified stop does not have any reviews.', 
                    'responseCode': 400
                }, 400
            ...
            return {
                'causeErrorMessage': str(e), 
                'causeErrorStatus': -1,
                'errorMessage': 'Cannot retrieve stop score: unknown SQLA exception.', 
                'responseCode': 500
            }, 500
            ...
            return {
                'causeErrorMessage': str(e), 
                'causeErrorStatus': -1,
                'errorMessage': 'Cannot retrieve stop score: unknown Python exception.', 
                'responseCode': 500
            }, 500
            ...
            return {
                'causeErrorMessage': 'N/A',
                'causeErrorStatus': -1,
                'errorMessage': 'Cannot retrieve stop score.', 
                'responseCode': 500
            }, 500
...
class GetReviewsForReviewer(Resource):
    ...
    def get(self, user_id):
        try:
            reviews = StopReview.query.filter_by(reviewer_id=user_id).all()

            return {
                "reviews": [
                    {
                        "stopId": review.stop_id,
...
class GetReviewsForStop(Resource):
    """
        API entrypoint to retrieve all reviews made for the specified stop.
    """
...
api.add_resource(StopReviewsHW, '/hello_world/')
api.add_resource(CreateReview, '/create/')
api.add_resource(GetScore, '/stop/<int:stop_id>/score/')
api.add_resource(GetReviewsForReviewer, '/reviewer/<int:user_id>/reviews/')
api.add_resource(GetReviewsForStop, '/stop/<int:stop_id>/reviews/')
